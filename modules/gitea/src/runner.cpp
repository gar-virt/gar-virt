...
public:
    ...
    static std::expected<Runner, GenericError> connect(RunnerOptions options,
        ...
        return Runner{
            runner.id(),       std::move(options.labels), std::move(options.forge_uri), std::move(credentials),
    ...
    const gitea::GiteaRunnerCredentials& credentials() const noexcept { return m_credentials; }
    const gitea::GiteaRunnerServiceClient& client() const noexcept { return *m_client; }
    const std::vector<std::string>& labels() const noexcept { return m_labels; }
    const std::string& forge_uri() const noexcept { return m_forge_uri; }
