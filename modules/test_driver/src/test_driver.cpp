...
struct failure_exit_codes {
    enum type { success = 0, failure = 1, exception_thrown = 2, no_tests_found = 3, test_not_found = 4 };
};
...
int cmd_run_test(const std::string& test_name) {
    ...
    try {
        ...
        std::cerr << "Test \"" << test.name() << "\" failed [" << info.condition << "] at " << info.file << ":"
                  << info.line << ".\n";
        exit_code = failure_exit_codes::failure;
    } catch (const std::exception& e) {
        std::cerr << "Test \"" << test.name() << "\" threw exception with message \"" << e.what() << "\".\n";
        exit_code = failure_exit_codes::exception_thrown;
    } catch (...) {
        std::cerr << "Test \"" << test.name() << "\" threw unknown exception.\n";
        exit_code = failure_exit_codes::exception_thrown;
    }
    return exit_code;
}
...
int cmd_run_all_tests() {
    ...
    for (const auto& it : tests) {
        ...
        name_width = std::max(name_width, test.name().size());
    }
    size_t success_count{};
    for (const auto& it : tests) {
        const auto& test{it.second};
        std::cout << test.name() << std::setfill('.') << std::setw(static_cast<int>(name_width - test.name().size()))
                  << "" << std::setw(0) << ": ";
        try {
            test.invoke();
            ++success_count;
