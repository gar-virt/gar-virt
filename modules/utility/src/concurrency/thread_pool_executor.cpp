...
class ThreadPoolExecutor::Impl final {
...
public:
    ...
    Impl(const std::optional<size_t>& min_thread_count, const std::optional<size_t>& max_thread_count)
    ...
    template <typename F> void put(F&& work) {
        {
            Lock lock{m_mutex};
            if (m_stop) {
                return;
            }
            m_queue.push(std::forward<F>(work));
...
ThreadPoolExecutor::ThreadPoolExecutor(const std::optional<size_t>& min_thread_count,
                                       const std::optional<size_t>& max_thread_count)
        : m_impl{std::make_unique<Impl>(min_thread_count, max_thread_count)} {}

ThreadPoolExecutor::~ThreadPoolExecutor() = default;
ThreadPoolExecutor::ThreadPoolExecutor(ThreadPoolExecutor&&) noexcept = default;
ThreadPoolExecutor& ThreadPoolExecutor::operator=(ThreadPoolExecutor&&) noexcept = default;
void ThreadPoolExecutor::stop() { m_impl->stop(); }
void ThreadPoolExecutor::cancel() { m_impl->cancel(); }
void ThreadPoolExecutor::put_impl(TaskFn work) { m_impl->put(std::move(work)); }
