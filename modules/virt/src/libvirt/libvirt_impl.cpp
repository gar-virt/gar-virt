...
public:
    ...
    static std::expected<std::unique_ptr<HypervisorImpl>, GenericError> connect(const std::string& uri) {
        auto loop_res{EventLoopImpl::get()};
        if (!loop_res) {
            return std::unexpected{loop_res.error()};
        }

        auto hv{std::make_unique<HypervisorImpl>(std::make_shared<ConnectionImpl>(uri), *std::move(loop_res))};

        auto reg_res{hv->register_event_handlers()};
        if (!reg_res) {
            return std::unexpected{reg_res.error()};
        }
...
Machine::Machine(std::unique_ptr<MachineImpl> impl) noexcept : m_impl{std::move(impl)} {}

Machine::~Machine() = default;

Machine::Machine(Machine&& other) noexcept : m_impl{std::move(other.m_impl)} {}
...
void Machine::notify_bad_state() { return m_impl->notify_bad_state(); }
void Machine::notify_ready() { return m_impl->notify_ready(); }

Hypervisor::Hypervisor(std::unique_ptr<HypervisorImpl> impl) noexcept : m_impl{std::move(impl)} {}
...
std::expected<std::shared_ptr<Hypervisor>, GenericError> Hypervisor::connect(const std::string& uri) {
    return HypervisorImpl::connect(uri).transform(
        [](auto&& impl) { return std::make_shared<Hypervisor>(std::forward<decltype(impl)>(impl)); });
}
